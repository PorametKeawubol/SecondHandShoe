...
export default function Message({ id, setShowModal }) {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (response2.data.Profile_Picture === null) {
        ...
        profile_picture2 = conf.urlPrefix + response2.data.Profile_Picture.url;
      }
      ...
      if (Array.isArray(response.data.data)) {
        ...
        const Message = MessagesData.filter((chat) => {
          return (
            ...
            (chat.Sender.id === receiverID && chat.Receiver.id === userData.id1)
          );
        });
  ...
  return (
    <div className="">
      <div className="fixed end-px inset-y-px z-0 w-[60%] h-[80%] mt-40 bg-opacity-80  overflow-y-auto  flex justify-center items-center">
        <div
          ref={modalRef}
              ...
              {Message &&
                Message.map((chat) => {
                  if (chat.Sender.id === userData.id1) {
                    return (
                      <MyMessage data={chat} pic={userData.profile_picture1} />
                    );
                  } else {
                    return (
                      <YourMessage
