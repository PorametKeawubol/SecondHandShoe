  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedImages.length > 0) {
        await Promise.all(
          selectedImages.map((index) => handleRemoveImage(index))
        );
      }
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label className="block mb-4">
              <span className="text-gray-700">
                Brand: {shoeData.brand && shoeData.brand.name}
              </span>
              <select
                name="brand"
            ...
            <label className="block mb-4">
              ...
              <select
                ...
                value={
                  editedData.color ||
                  shoeData?.color?.data?.attributes?.name ||
                  ""
                }
                onChange={handleInputChange}
                className="mt-1 block w-full rounded-md border-black border shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
            ...
            <label className="block mb-4">
              ...
              <select
                ...
                value={
                  editedData.gender ||
                  shoeData?.gender?.data?.attributes?.name ||
                  ""
                }
                onChange={handleInputChange}
                className="mt-1 block w-full rounded-md border-black border shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
            ...
            <div className="mb-4">
              <label htmlFor="usSize" className="text-sm text-gray-600">
                US Size:
              </label>
              <select
                id="usSize"
                  ...
                  shoeData.picture.data.map((image, index) => (
                    <div key={index} className="relative mr-4 mb-4">
                      <div className="relative">
                        <img
                          src={`${conf.urlPrefix}${image.attributes.url}`}
                          alt={`Image ${index}`}
